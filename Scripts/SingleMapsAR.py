...
from Deconvolution import AutoRegressiveAlgorithms

path = './Data/NewTHz.mat'
SNR_dB = 20
SAVE_PATH = f'./Maps/DataTHz/AR_NewTHz_{SNR_dB}dB.mat'
...
for _ in tqdm.tqdm(range(NMean)):
    ...
    for i in range(len(Signals)):
        ...
        # Algorithms 
        start = time.time()
        aAR,tAR,SignalAR = AutoRegressiveAlgorithms.ARSE(Signals[i],Reference,2,BW=0.1)
        stop = time.time()
