...
from Deconvolution import SubspaceAlgorithms
import os
...
SAVE_PATH = f'./Maps/DataTHz/MUSIC_NewTHz_{SNR_dB}dB.mat'
...
RayleighSignal = Helper.GenerateSignal(Reference,0.1,0.1*FWHM)
...
for _ in tqdm.tqdm(range(NMean)):
    ...
    for i in range(len(Signals)):
        ...
        # Algorithms
        start = time.time()
        aMUSIC,tMUSIC,SignalMUSIC = SubspaceAlgorithms.MUSIC(Signals[i],Reference,2)
        stop = time.time()
        Time.append(stop-start)
        
        # Save Data
        TauR.append(tMUSIC)
        AlphaR.append(aMUSIC)
        SignalR.append(SignalMUSIC)
