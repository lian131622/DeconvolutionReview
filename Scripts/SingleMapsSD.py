...
import tqdm
from Deconvolution import ISTAlgorithms
import Helper

path = './Data/NewTHz.mat'
SNR_dB = 20
SAVE_PATH = f'./Maps/DataTHz/SD_NewTHz_{SNR_dB}dB.mat'
...
# Choose step-size
mu = 1/np.linalg.norm(X.T@X,2)

_, _, _, l, _, LCurve = ISTAlgorithms.OptimizedL1(RayleighSignal, Reference, 2, mu)

sigma = l[Helper.FindKnee(LCurve[0],LCurve[1])]
...
for _ in tqdm.tqdm(range(NMean)):
    ...
    for i in range(len(Signals)):
        ...
        # Algorithms 
        start = time.time()
        aL1,tL1,SignalL1 = ISTAlgorithms.L1(Signals[i],Reference,2,mu,sigma,N_iter=100)
        stop = time.time()
...
mu = 0.00031935383826219683
sigma = 12.648552168552959
Signal = Helper.GenerateSignal(Reference,-0.6,0.15*FWHM)
aL1,tL1,SignalL1 = ISTAlgorithms.L1(Signal,Reference,2,mu,0.5*sigma)

plt.plot(Signal)
plt.plot(SignalL1/np.max(np.abs(SignalL1)))
plt.show()